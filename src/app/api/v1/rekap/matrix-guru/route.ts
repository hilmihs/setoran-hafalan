...
import { rekapMatrixGuru } from '@/lib/api-public/rekap';
...
export async function GET(req: NextRequest) {
  return handle(async () => {
    const pre = await rekapPreamble(req, 'penilaian');
    ...
    const key = `rekap/matrix-guru?bulan=${bulan}&gender=${gender ?? ''}|${pre.scopeKey}`;
    return serveCached(key, pre.ifNoneMatch, () => rekapMatrixGuru(bulan, gender ?? undefined));
  });
}
