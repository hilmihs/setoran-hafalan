...
export default async function SyaikhPenilaianPage() {
  const s = await getSession();
  const session = s.session;
  if (!session || session.role !== 'syaikh') {
    redirect('/');
  }
  ...
  const { data: allPeserta } = await supabaseAdmin
    .from('peserta')
    .select('id, name, gender, kelas_id, kelas:kelas_id(id, name)')
    .eq('active', true)
    .order('name');
  ...
  const kelasMap = new Map<string, { id: string; name: string }>();
  for (const p of allPeserta ?? []) {
    const k = p.kelas as unknown as { id: string; name: string } | null;
    if (k && !kelasMap.has(k.id)) kelasMap.set(k.id, k);
  }
  ...
  return (
    <main style={{ padding: '0 0 80px' }}>
      <div className="page-header">
        <Link href="/2in1/syaikh" className="back-btn" aria-label="Kembali">←</Link>
        <div>
