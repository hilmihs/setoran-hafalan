...
import { approveHapus, rejectHapus } from './actions';
...
export function DecidePanel({
    ...
    return (
      <div className="card-flat" style={{ padding: '16px', textAlign: 'center' }}>
        <p className="t-body" style={{ fontWeight: 600, color: done === 'approved' ? 'var(--hijau-ink)' : 'var(--danger)' }}>
          {done === 'approved' ? 'Pengajuan disetujui — pertemuan dihapus.' : 'Pengajuan ditolak.'}
        </p>
        {catatanKoordinator && (
  ...
  function decide(kind: 'approve' | 'reject') {
    ...
    startTransition(async () => {
      const res = kind === 'approve' ? await approveHapus(token, catatan) : await rejectHapus(token, catatan);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          ...
          style={{ flex: 1 }}
        >
          {pending ? 'Memproses…' : 'Setujui & hapus'}
        </button>
        <button
          type="button"
          className="btn-ghost"
