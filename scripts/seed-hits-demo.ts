/**
 * Seed data HITS untuk demo.
 *
 *   pnpm seed-hits
 *
 * Akan membuat:
 *   - 19 kelompok pengajar (6 ikhwan + 13 akhwat)
 *   - 90+ pengajar beserta ketua kelompok
 *   - 5 koordinator HITS
 *   - 2 koordinator ketua kelas
 *   - 2 program kehadiran (At-Tibyan, Muallim Najih)
 *   - Demo: beberapa checkin, pengajuan alasan, libur
 *
 * Password default: "hits123"
 * Re-run aman — script hapus data lama dulu.
 */
...
const AKHWAT_KELOMPOK: KelompokData[] = [
  ...
  },
  {
    ...
    anggota: [
      ...
      // Silmi Muthmainnah skip — sudah di Kelompok 1
...
const del = (table: string) =>
  supabaseAdmin.from(table).delete().neq('id', NIL);

async function main() {
  console.log('Hashing password default…');
  const hash = await bcrypt.hash(PWD, 12);

  console.log('\n🗑  Membersihkan data HITS lama…');
  await del('audit_log');
  ...
  console.log(`  ✓ ${koorHits!.length} koordinator HITS`);

  // ── Koordinator Ketua Kelas ──
  console.log('\n🎓 Insert koordinator ketua kelas…');
  ...
  console.log(`  ✓ ${koorKK!.length} koordinator KK`);

  // ── Kelompok & Pengajar ──
  console.log('\n👥 Insert kelompok pengajar & pengajar…');
  ...
  for (const kel of ALL_KELOMPOK) {
    ...
    for (const a of kel.anggota) {
      ...
      if (seenWa.has(wa)) {
        console.log(`    ⚠ Skip duplikat: ${a.name} (${wa})`);
        continue;
      }
      seenWa.add(wa);
      rows.push({
    ...
    process.stdout.write(`  ✓ ${kel.name} (${rows.length} pengajar)\n`);
  }
  console.log(`  Total: ${totalPengajar} pengajar`);

  // ── Demo data: Checkin ──
  console.log('\n📝 Insert demo checkin data…');
  ...
  if (somePengajar && somePengajar.length >= 3 && tibyanId && muallimId) {
    ...
    if (ciErr) console.log(`  ⚠ Checkin error: ${ciErr.message}`);
    else console.log(`  ✓ ${checkins.length} demo checkin`);

    // Demo pengajuan alasan
    const alasan = [
      {
        pengajar_id: somePengajar[1].id,
        program_id: tibyanId,
        tanggal: yesterday,
        jenis: 'terlambat',
      ...
      {
        pengajar_id: somePengajar[2].id,
        program_id: tibyanId,
        tanggal: yesterday,
        jenis: 'alpa',
        alasan: 'Ada keperluan keluarga mendadak yang tidak bisa ditunda.',
        status: 'accepted',
        decided_by: somePengajar.find((p) => {
          // cari ketua di kelompok yang sama
    ...
    if (aErr) console.log(`  ⚠ Alasan error: ${aErr.message}`);
    else console.log(`  ✓ ${alasan.length} demo pengajuan alasan`);
  }
  ...
  if (tibyanId) {
    ...
    if (libErr) console.log(`  ⚠ Libur error: ${libErr.message}`);
    else console.log(`  ✓ 1 demo libur (${saturdayStr})`);
  }

  // ── Summary ──
  console.log('\n' + '='.repeat(50));
  console.log('✅ Seed HITS demo selesai!');
  console.log('='.repeat(50));
  console.log(`\nPassword default semua akun: "${PWD}"`);
  console.log('\nAkun demo untuk login:');
  console.log('  Pengajar:          081399741809 (Hilmi Hanif Sobandi)');
  console.log('  Ketua Kelompok:    082199266821 (Muhammad Sofyan)');
  console.log('  Koordinator HITS:  081280672014 (Abdul Muhsin)');
  console.log('  Koordinator KK:    081280630437 (Koordinator KK Ikhwan)');
  console.log('\nGanti password via: pnpm set-password');
}
