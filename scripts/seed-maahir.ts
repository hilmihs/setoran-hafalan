 ...
 *
 * Script ini akan WIPE data rekaman/setoran/peserta/kelas/musyrif terlebih dulu,
 * lalu insert ulang. Koordinator tidak disentuh.
 *
 * Cara pakai:
 *   npm run seed-maahir
 *
 * Default password semua musyrif: "password123" — ganti via `npm run set-password`.
 */
...
async function main() {
  ...
  console.log('\n[1/4] Membersihkan data lama (rekaman, setoran, peserta, kelas, musyrif)…');
  ...
  musyrifInserted!.forEach((m) =>
    console.log(`  ✓ ${m.name} (${m.whatsapp_number})`)
  );
  ...
  kelasInserted!.forEach((k) => console.log(`  ✓ Kelas ${k.name}`));
  ...
  for (const k of KELAS) {
    ...
    if (pErr) throw pErr;
    console.log(`  ✓ Kelas ${k.name}: ${rows.length} peserta`);
    totalPeserta += rows.length;
  }

  console.log(`\n✓ Selesai. ${musyrifInserted!.length} musyrif, ${kelasInserted!.length} kelas, ${totalPeserta} peserta.`);
  console.log(`Default password semua musyrif: "${DEFAULT_PASSWORD}"`);
  console.log(`Ganti dengan: npm run set-password`);
}
